...
A setting, off by default, opens a card's pull request beside its issue: the issue on the left, the pull request on the right, each scrolling on its own, with the same header, sticky bar, and controls per side. The pair is one panel, sized from its left edge as the single panel is, with a floor of two readable conversations and a width of its own, retained apart from the single panel's and defaulting to nearly the whole window. The issue control still opens the issue alone, so the pair is reached from the pull request only. A card whose pull request has no issue cannot occur: every card on the board is an issue. Links inside either side keep their behavior; neither side retargets the other. An answer for one side redraws that side alone, so the other keeps its scroll and focus.
...
The overlay runs on GitHub, which opens a card's issue in a side panel of its own, so the overlay leaves issues to that panel. GitHub opens a card's pull request in a new tab instead. The overlay takes that click and opens the pull request in a side panel of the same shape as GitHub's issue panel: under the site header, over a dimmed board, sliding in from the right at GitHub's width, closed by its close control, the backdrop, or Escape, with focus returned to the link. Nothing outside the panel takes pointer or keyboard while it floats, the site header included, where GitHub's panel leaves its header reachable. The panel frames GitHub's own pull request page with the site header, repository header, and footer hidden, so it shows everything the page shows, including the diff, and lets the developer write there as they could on the page; this is the one place a client writes to GitHub, and it is GitHub's page doing the writing. Admitting a page into a frame costs the browser-side policy on issue pages the developer opens in a tab while logged in (mechanics M58, architecture: header rule); that is accepted for the pair rather than leaving it unable to frame an issue. Its bar names the repository and number and carries the controls: copy the address, pin the panel, open the same page in a new tab, and close. Once the framed page is scrolled far enough for its own sticky header to show, that header carries the controls and the bar is hidden, so the scrolled panel is one row with the state, the title, and the controls, as GitHub's own panel is; the bar returns when the header unsticks, and a control that holds focus keeps it across the move. Inside the frame the controls keep their accessible names but show no tooltip, since the overlay's tooltip belongs to the board page; the framed page shows none for its own header either. Header and bar are the panel's own; the page inside the frame scrolls as the page does. Pages of the same pull request open inside the frame; any other link opens a new tab, because only pull request pages are admitted into the frame (architecture: header rule). Holding Ctrl or Cmd, or using another button, leaves the click to GitHub.

With pairing on, the same click frames the card's issue on the left of the pull request, each page in a frame of its own with its site chrome hidden, so the issue too shows everything its page shows. The pair has no bar, since each framed page names itself; the controls float at the top right over the pull request page and act on it. The pair floats only: the pin control is not offered, and GitHub's own pin choice is left as it was, though an open pinned issue pane is closed as it is for a single panel and does not come back when the pair closes. A link off either page opens a tab, the pull request's own pages staying in its frame and the issue's in its own, so neither frame retargets the other. A card with no issue link opens the pull request alone.

Pinned, the panel docks beside the board as GitHub docks its own issue panel: the backdrop goes, the board keeps the width the panel leaves it and stays live, Escape and the backdrop no longer close it, and the close control does. The pin choice and the docked width are the ones GitHub's issue panel keeps, so the two panels behave as one pane: a pull request opened beside a pinned issue closes the issue pane and opens docked at its width and place, and an issue opened beside a pinned pull request closes the pull request panel and opens GitHub's pane at the same width; pinning or sizing either side sets both. The content of either panel fills the width it is given: GitHub's own 1280px column cap in its issue panel, and the pull request page's content caps inside the frame, are lifted. Both the floating and the docked panel are sized from their left edge, by pointer and by arrow key, within bounds that keep the panel readable and the board present; each mode keeps its own width in the browser. GitHub sizes its issue panel only once pinned; the overlay gives the floating issue panel the same edge, and that width is the one the floating pull request panel opens at, so the two panels read as one. There is no setting to turn the panel off and no address bar state, which the issue panel has and the overlay does not reproduce.

### R47. Custody: where an issue has been

A card's issue has a status and an assignee, which say where it is. Neither says how it got there: the history is in the issue timeline, interleaved with comments and label churn, and GitHub renders a status change as one line with no duration. The custody popup answers that from the card, in a frame small enough to read without scrolling, in both clients.

It opens from the card's issue number on every card with an issue: on the editor board the number is a button whose text stays its accessible name, with `Show custody: where this issue has been and who held it.` as its tooltip and description; on GitHub the overlay makes the card's own number line that control, since GitHub gives it no click of its own and the card's name reads from it. The card title opens the conversation (R43). The popup hangs from the number like a lane menu: one at a time, closed by pressing the number again, clicking outside, or scrolling or resizing the board, and on the editor board also by Escape, which returns focus to the number; the editor board focuses the popup as it opens. It stays on its control across a redraw and goes when its card leaves the board. Opening asks the hub for the card by key; the popup shows the card's number and title with `Reading timeline…` until the hub answers, a failure in the hub's words, or `GitHub has no timeline for this issue.` when the source found nothing.

The hub folds the issue's timeline into **legs**: one stretch of time in one status with one set of assignees. A leg opens at creation and at every hand-off, where a hand-off is one actor's status change and assignment within a minute of each other, which is how the board records a move. Legs under fifteen minutes fold into the leg before them (the first into the one after), and equal neighbours merge, so creation flicker and an automation's bounce leave every tab. The current leg is never dropped. The last leg closes at the close time of a closed issue and at the time of the read otherwise; a status moved after the close opens nothing. Linked accounts (R28) resolve before anything else, so a linked bot is its person, held once however many of their accounts are assigned and until the last is unassigned; a login the `custody.bots` setting names, or one ending in `[bot]`, is a machine whose moves count as automated.

`custody.stages` lists the board's statuses in workflow order with the function that owns each — intake, dev, product, qa, or release — and the hold length that reads as a stall there. A move to an earlier stage is a send-back. A status outside the list still renders, ranks nowhere, and never counts as a send-back. `statusLanes` cannot supply this: it maps three statuses to lanes and carries no order.

Above the tabs, every popup carries the same header and bar: the number, an Open or Closed pill, the age as `30d old`, the title on at most two lines, and the **custody bar** — the issue's whole life as one strip, each leg a segment whose width is its share of that life, at least two pixels, coloured by function and faded when nobody held it, with `status · holder · duration` on hover; the creation date sits under the left end, with its year when the life spans two, and `closed` or `today` under the right. The bar is linear time: a long stall leaves the other legs a few pixels wide, and that is the fact it shows. A read the source cut short says so under the bar.

Three tabs share that header; the chosen one is kept for the next popup in each client. Every word on them is the hub's, so both clients show the same text.

| Tab | Question | What it shows |
|---|---|---|
| Health | Is this stuck, and how badly? | One headline, red when bad, chosen by the first rule that matches: a leg at or past its stage's stall floor (`Stalled 8.2d in New`, the longest such leg), two or more human send-backs (`Sent back 2 times`), a closed issue (`Closed after 26d`), an unassigned current leg past three days (`Unowned for 4.0d`), else `Moving normally`, which an issue under an hour old always reads. A subline names the longest leg by status, duration, and share of life, never by person. Four figures: hand-offs, people, sent back (red from one; automated ones beside it as `+1 auto`), and unassigned share (red from half). A Now strip: the current status, holder or `unassigned`, and time in that state. |
| Time | What consumed its life? | The four longest legs — status, holder, duration (red past seven days), and a track scaled to the top row — then up to five statuses by total time, each with a track scaled to the issue's life. Order and hand-offs are not shown here; the bar above is the only ordered element. |
| Route | How did it get here, through whose hands? | One stop per run of consecutive legs in one status, oldest first on a rail: a dot in the function's colour, the status, the duration, and the holders in first-hold order with bots left out. An unbroken alternation of two statuses, three stops or longer, is one loop stop, `Testing ⇄ Dev ×2` with the arrows and count in red, its dot split between the two functions, and each holder on the side of the status they held longer (`qa-1 with dev-1`, or `dev-1 alone`). The same pair separated by another status is two loops, because folding them would misstate the order. The last stop is the current one, in bold with `now`, its dot ringed; a closed issue nobody holds names who closed it. Past eight stops the first three and last four stay and the middle folds into one grey `{n} more stops` row with its summed time and its people, because the reader is usually asking how the issue reached its current stop. |

A stall outranks a close, because the reader of a closed issue is usually learning from it. Health names statuses and durations and never a person, because the board is shared and a sentence with a name in it reads as blame; Time and Route name people, because there a name is a fact in a list. The stall floors are per function rather than a share of life, because a short healthy issue spends most of its life in one place too.

The hub reads the whole timeline oldest first, every page, because a read that stops early would lose the first leg and make every share wrong; a page that fails after the first leaves the history short and marked. The history is cached per card and `updatedAt`, since every timeline event moves `updatedAt`, and folded on every request, so an open issue's ages run on; the cache is bounded. Nothing is precomputed for the board: a popup is one request.

Both clients open the popup from the card. An icon in GitHub's own issue sidebar is not offered: the overlay draws inside project-board cards, and the issue page is a different application (mechanics M57). Clicking inside the popup opens nothing in this version; the annotated conversation the tabs would scroll to is a separate requirement.
...
`github.linkedAccounts` maps a login to the login shown in its place: `{ "jon-or-ai": "jon-or" }` shows everything the bot did as jon-or, with jon-or's profile name and face, wherever the hub reads it from GitHub — card avatars, assignees, pull-request authors, the conversation panel, and the triage context. The hub substitutes before data reaches a client or the classifier; the recorded login survives only as provenance, shown on hover and in the accessible name (R5), and as `as jon-or-ai` on hover and in the description of each substituted name in the conversation panel, never in running text or in plain login lists. Every login list is deduplicated after substitution, and a developer assigned beside their bot reads as directly assigned.

The link declares whose work it is. The configured logins resolve through the links before they identify the developer, so a bot named alone in `github.logins` and linked to jon-or still owns its pull requests and lands in the review lane as jon-or; `github.logins` must still name the bot for its cards to load. A bot in `github.logins` linked to a colleague makes the colleague a developer identity, which the hub warns about once. Links resolve one step: a link to a login that is itself linked, a link to itself, or either side of a mutual pair is dropped with a warning, and one unreadable entry never refuses the rest of the settings.

The target's profile is read once a day through one request per distinct target. While it is unread, or when GitHub does not know the login, the recorded face stays under the target's login and the hub warns once, retrying after an hour; a refresh that fails keeps the profile already held. The configured logins are resolved the same way when the card avatar prefers the developer among the assignees (R5). Renaming a colleague without a link is not supported; a link is to a login and the name is that login's profile name. The overlay substitutes only where it already replaces GitHub's markup, the card avatar; GitHub's own assignee stack and conversation page keep the recorded account.
