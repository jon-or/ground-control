...
`avatar` selects whose face a card shows in both clients. Under `review-author`, the default, and `issue-author`, a card whose status is mapped to the Review lane shows the selected pull request's author. Off review, `issue-author` shows the issue author and `review-author` shows an assignee. `assignee` always shows an assignee. An assignee is shown wherever the policy's person is unavailable, such as a review card with no pull request or a deleted account, and the preferred assignee is the developer's configured identity. Label the role. The review statuses come from the lane mapping, so custom status names need no second list. The overlay replaces GitHub's assignee display only where an author should replace it and an assignee area already exists; otherwise leave GitHub's display intact.
