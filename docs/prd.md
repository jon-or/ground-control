...
A card that has been triaged also carries what it is waiting on and one sentence about the work (R38), which is the only thing on a card the developer cannot read off the rest of it.
...
A card the developer has not looked at is a title and a status; what it actually wants from them is several clicks away, in a conversation they have to read. So when a card arrives on the board — for the first time, or coming back after leaving — the board reads the recent conversation on the issue and on the pull request it is showing, and labels the card with one action and one sentence describing the work. **Begin work**, **Address dev review · followup**, **UAT failure**, **Land it**: enough to decide whether this is the card to pick up, which is what R5 asks of everything else on the card.
...
**What can be read as fact is never guessed.** A branch that will not merge, one behind its base, red checks, an approved and clean pull request, and whether a review round is a first or a later one are all read from GitHub directly, and that reading outranks anything anybody wrote.

**A problem somebody reported still counts before GitHub has computed it.** Mergeability is not computed at the moment a card arrives, which is exactly when a card is read — so a failed auto-merge, a request to rebase, and a red build are things the classifier may name from the words on the card, and commonly the only evidence there is. What it may never say is that everything is fine: a problem somebody reported is evidence, and "ship it" is an opinion where mergeability is a fact. Where a fact then overrules a reading, it takes the sentence with the label, because a card reading "Land it" over "has merge conflicts to resolve" is a card contradicting itself. Nothing is read off a draft, off somebody else's pull request, off one already landed, or off a mergeability GitHub has not computed yet — which is common in the moment a card arrives, and means "not computed", never "fine". **Merge upstream** only ever appears where the repository requires branches to be up to date; where it does not, its absence says nothing.

**It says when it decided.** A card is read once, so a label ages. The board marks a reading as aged when the card has visibly moved under it — a pull request opened, a status or a review decision changed — and, because much of what moves a card is invisible to it, when the reading is simply old. It gives the age either way rather than presenting a Monday answer as today's.
...
**Classifying sends the card's text off the machine.** The issue and pull request titles, bodies and recent comments go to the model's API, because inference is not local — other people's comments included, and with them who wrote each one, how they relate to the repository, and what any review said. Everything else the board does stays on this machine. This is the one thing it does that does not, it is on by default, and one setting turns it off.
