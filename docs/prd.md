...
| Tail | Time in the current status, right-aligned | Reread, open checkout, create worktree (R46), run, custody (R47) — custody on the edge |
...
It opens from a **Show custody** control in the command bar's tail (R45), after the run control, on every card with an issue. The popup hangs from that control like a lane menu: one at a time, closed by pressing the control again, clicking outside, or scrolling or resizing the board, and on the editor board also by Escape, which returns focus to the control; the editor board focuses the popup as it opens. It stays on its control across a redraw and goes when its card leaves the board. Opening asks the hub for the card by key; the popup shows the card's number and title with `Reading timeline…` until the hub answers, a failure in the hub's words, or `GitHub has no timeline for this issue.` when the source found nothing.

The hub folds the issue's timeline into **legs**: one stretch of time in one status with one set of assignees. A leg opens at creation and at every hand-off, where a hand-off is one actor's status change and assignment within a minute of each other, which is how the board records a move. Legs under fifteen minutes fold into the leg before them (the first into the one after), and equal neighbours merge, so creation flicker and an automation's bounce leave every tab. The current leg is never dropped. The last leg closes at the close time of a closed issue and at the time of the read otherwise; a status moved after the close opens nothing. Linked accounts (R28) resolve before anything else, so a linked bot is its person; a login the `custody.bots` setting names, or one ending in `[bot]`, is a machine whose moves count as automated.
