...
The board's own machinery is the one exception. When the board classifies a card (R38) it runs an agent process of its own, and that process is not the developer's work — it is the board reading. It is kept off the board deliberately, and it is the only session that ever is.
...
**What can be read as fact is never guessed.** A branch that will not merge, one behind its base, red checks, an approved and clean pull request, and whether a review round is a first or a later one are all read from GitHub directly. Only what is genuinely a judgement about what people wrote is classified. Nothing is read off a draft, off somebody else's pull request, off one already landed, or off a mergeability GitHub has not computed yet — which is common in the moment a card arrives, and means "not computed", never "fine". **Merge upstream** only ever appears where the repository requires branches to be up to date; where it does not, its absence says nothing.

**It says when it decided.** A card is read once, so a label ages. When the card has moved under the reading — a pull request opened, a review landed — the label says so and gives its age rather than presenting a Monday answer as today's. The developer can ask for any card to be read again.
...
**Classifying sends the card's text off the machine.** The issue and pull request titles, bodies and recent comments — other people's included — go to the model's API, because inference is not local. Everything else the board does stays on this machine. This is the one thing it does that does not, it is on by default, and one setting turns it off.
...
In their own terms: what is missing, or what failed. Where that fits a badge, the badge carries it; a condition that belongs to the whole board — a status set matching nothing, a source that failed to read, hooks that could not be installed — is stated once above the lanes rather than on every card. A card carries one explanation of its own and no more: the sentence triage writes about what the work is (R38). The per-card hover that used to explain why a card could not move was removed and nothing replaced it — that was a restatement of what the card already showed, where the triage sentence is the one thing on the card the developer cannot read anywhere else on it. A triage that failed writes nothing there, so a card never explains its own absence of an explanation.
