...
**A detached run has a path of its own, and it stops here.** `vscode://groundcontrol.ground-control/attach?session=<id>` carries no agent and no hop, because nothing is routed: the window that receives the navigation resolves the id against its own board or the hub's roster and opens a terminal itself. Measured 2026-09-09 in a real extension host: `vscode.window.createTerminal({ name, cwd, shellPath: 'claude', shellArgs: ['attach', '<short id>'] })` reports both back on `creationOptions` and resolves a `processId`, so the CLI is the terminal's own process and a configured path with a space in it is one argument rather than two tokens for a shell to split. `shellPath` takes no PATHEXT search, so an install that puts a `.cmd` on the path rather than an executable would need the path named in `groundControl.agents`.
