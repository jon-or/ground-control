...
The `cwd` on `user`/`assistant` records follows the Bash shell, not the session (files, 2026-09-11). A session launched in a repository that ran `cd` into a worktree recorded the worktree on its later records while its transcript stayed under the repository's project directory, and a resume ran in the repository. `EnterWorktree` is what moves a session: it moves the transcript to the worktree's project directory and writes `{"type":"relocated","relocatedCwd":…}`, re-emitted on every later turn (270 records for 2 calls in one session), and every later shell reset in 22 relocated sessions targeted the worktree; `ExitWorktree` writes the same record naming the repository. Relocated sessions whose last turn ran `cd` back to the repository still recorded the repository as their last cwd. The resume directory is therefore the last `relocated` record, else the launch record's cwd, the rule the VS Code extension's own listing applies ([M52](#claude-session-working-directories)). Over 748 local transcripts the last-cwd rule disagreed with the transcript's directory in 54; this rule in 2, both probe sessions launched with a worktree cwd but no relocation. The launch record was inside the 64 KiB head window in 747 of 748; past it, the first complete tail record stands in.
...
### GitHub conversation page

**Record M57. Runtime/DOM, 2026-09-11, public issue and pull-request pages in both colour schemes, Chromium via Playwright. Used by the conversation panel (R43).**

Body type is 14px on a 21px line. The title is 32px at weight 400 on a 40px line, with the number after it at weight 300 in `--fgColor-muted`. The state pill (`StateLabel`, medium) is 32px tall, 8px 12px padding, 14px at weight 600 on a 16px line, full radius, white on the emphasis colour, its 16px octicon 4px before the word: open `--bgColor-success-emphasis` #1f883d/#238636, merged and closed-as-completed `--bgColor-done-emphasis` #8250df/#8957e5, closed pull request `--bgColor-danger-emphasis` #cf222e/#da3633, draft and not-planned `--bgColor-neutral-emphasis` #59636e/#656c76. Branch names are 12px monospace on an 18px line, 2px 6px padding, 6px radius, `--fgColor-accent` on `--bgColor-accent-muted` #ddf4ff/#388bfd1a. Sidebar headings are 12px at weight 600 on an 18px line in `--fgColor-muted`, 8px above their value.

A comment box has a 1px `--borderColor-default` border and 6px radius. Its header is `--bgColor-muted` with the same border below, 4px 8px padding on the issue page (the pull-request page still uses the older 0 16px header with a 40px avatar outside the box), a 24px avatar, the author at weight 600 in `--fgColor-default`, and the time in `--fgColor-muted`. The body has 16px padding. Reactions are 28px pills, 0 8px padding, 1px `--borderColor-default`, `--fgColor-muted` text, inside the box below the body.

Timeline events sit on a 2px vertical line in `--borderColor-muted` (#d1d9e0b3/#3d444db3) at the badge's centre. A badge is a 32px disc on `--bgColor-muted` with a 2px ring in the page colour and a 16px octicon in `--fgColor-muted`; an approval fills it with the success colour, a request for changes with the danger colour, a merge with the done colour, and the octicon turns white. Event text is 14px in `--fgColor-muted` with the actor at weight 600, a 16px avatar before it, and 16px between rows. A review thread is a bordered box whose header is 8px padding on `--bgColor-muted` with a chevron, the path in 12px monospace, and "Show resolved" on the right; its comments have 16px padding and no border of their own.

Markdown bodies: paragraphs and lists end in 16px; headings are weight 600 on a 1.25 line with 24px above and 16px below, `h1` 2em and `h2` 1.5em ruled below in `--borderColor-muted`; inline code is 85% monospace with 0.2em 0.4em padding and 6px radius on `--bgColor-neutral-muted`; code blocks have 16px padding on `--bgColor-muted`; blockquotes have a 0.25em `--borderColor-default` rule and `--fgColor-muted` text; table cells are 6px 13px with `--borderColor-default` borders and even rows on `--bgColor-muted`; task-list checkboxes pull back 1.4em into the list gutter.

Labels: light mode fills with the label colour and picks black or white text by a perceived-lightness threshold of 0.453 (`--label-r/g/b/h/s/l` set inline); dark mode fills at 18% alpha, borders at 30%, and lightens the text by the shortfall to the threshold. A #d73a4a label reads white on the full colour in light and #b3969f-class lightened text on an 18% tint in dark. The panel reproduces both formulas.

The issue page's sticky header (`#issue-viewer-sticky-header`) is `position: sticky; top: 0`, 56px tall, `--bgColor-default` with a 1px `--borderColor-default` rule below, `display: none` until the whole page header (title and metadata row) has scrolled past, then a 32px flex row 8px apart under 12px 24px padding: the medium state pill, the title at 14px weight 600 on a 21px line clipped to one line with an ellipsis, the number at 14px weight 400 in `--fgColor-muted` 4px after it, a 12px metadata row under them in `--fgColor-muted` (shown from 768px wide) with `•` between its parts, and the 32px icon controls on the right, 4px apart. The panel draws the same bar over its own scrolling body.

The pull-request page and the issue page are different applications: the issue page (React, `prc-*` classes) puts the avatar inside the comment header and lays events with 16px avatars, while the pull-request page (`TimelineItem`, `timeline-comment`) puts a 40px avatar beside the box. The panel follows the issue page for both subjects because the inline avatar fits a narrow panel.

### GitHub side panel and framed pull request pages

**Record M58. HTTP headers, runtime/DOM, and Chromium 151 via Playwright 1.62, 2026-09-11, public roadmap board and public pull request pages. Used by the overlay's pull request panel (R43).**

Every github.com page answered with `X-Frame-Options: deny` and a `Content-Security-Policy` carrying `frame-ancestors 'none'`, a public pull request page included; a same-origin `iframe` on a github.com page was refused and Chrome navigated it to `chrome-error://chromewebdata/`, whose `contentDocument` reads null. An unpacked extension with the `declarativeNetRequest` permission, a `https://github.com/*` host permission, and one static `modifyHeaders` rule removing both headers for `sub_frame` requests initiated by `github.com` let the same frame load: the page rendered, ran its scripts, did not break out of the frame, and its document was readable from the parent. The rule applied to responses from the network and not to responses Playwright's `route.fulfill` synthesized, which stayed refused; the repository test serves the page from a loopback TLS listener that `--host-resolver-rules=MAP github.com 127.0.0.1:<port>` sends `github.com` to, with a self-signed pair for the name.
...
Pin side panel replaced the dialog with a `PageLayout.Pane` (`role=region`, `data-resizable`, the same `Side panel: Issue: <title>` name) docked at the right from the offset down, 320px wide at first, with no backdrop and the board reflowed beside it; `localStorage` gained `projects.sidePanelPinned=true` and the control became Unpin side panel with `octicon-pin-slash`. Its left edge is a `PageLayout.DragHandle` (`role=slider`, `aria-label` "Draggable pane splitter", `aria-valuemin` 256, `aria-valuemax` 541, `aria-valuetext` "Pane width N pixels"), 9px wide straddling the edge with `cursor: col-resize`, tinted `rgba(129, 139, 152, 0.12)` while held. A drag set `--pane-width` on the pane and `projects.sidePanelWidth` in `localStorage` to the new width; ArrowLeft with the handle focused changed the width by 3px. The floating panel is not resizable: its sheet's only width is the inline `--side-panel-width: min(90%, 1280px)` variable, which the sheet's stylesheet reads as its width at 768px and above, and which the overlay overwrites to size it. Both forms lay their content in a `ContentWrapper-module__contentContainer__<hash>` column with `max-width: 1280px` and 24px side padding, so a wider panel leaves the rest empty; the overlay lifts the cap. On a later visit GitHub read `projects.sidePanelPinned` to open the pane pinned and `projects.sidePanelWidth` as its `--pane-width` (640 from storage), but the pane's inline `--pane-max-width` (563px in a 1522px window; the 541 above was another window) clamps the drawn width, and the drag handle's `aria-valuemax` keeps that cap, so a drag of GitHub's own handle, or a window resize that makes Primer rewrite the variable, brings a wider width back under it until the overlay's next paint; setting the inline variable higher let the stored width draw, and a pane closed by its Close panel control stayed in the page, named `Side panel` alone, 0px wide, with its inline width intact, so reopening an issue drew it at the same width. The pane's Close panel, Copy link, and Unpin side panel controls are named by `aria-labelledby` tooltip spans, not `aria-label`.

The same rule, widened to `/issues/<n>`, let a synthetic issue page served with both refusal headers into a frame beside the pull request frame (2026-09-11, the repository's browser suite), and on 2026-09-12, logged out in headless Chromium 151, a live public issue page (`microsoft/vscode/issues/1`) framed from the extension's isolated world on the public roadmap board loaded and yielded its `h1`; `chrome.declarativeNetRequest.isRegexSupported` accepts the widened pattern. The same frame inserted from the page's own world was refused with or without the rule, because the board page's own `Content-Security-Policy` names only `viewscreen.githubusercontent.com` and `notebooks.githubusercontent.com` in `frame-src`; a frame inserted by a content script is checked against the isolated world's policy, not the page's, which is why the panel's frames load at all. Logged in as jon-or in the same Chromium on 2026-09-12, both a public and a private issue frame were refused with `frame-ancestors 'none'` while a pull request frame loaded: logged-in GitHub registers a service worker at `https://github.com/` that answers issue page navigations by fetching them itself, `response.fromServiceWorker()` true, and Chrome reports that fetch to `declarativeNetRequest` as `xmlhttprequest`, not `sub_frame`, so the rule never matched it (`onRuleMatchedDebug`, empty); a cache-busting query changed nothing. A rule on `sub_frame` and `xmlhttprequest` matched the worker's fetch and the private issue framed with its `h1`. The same worker proxies top-level issue page visits, which under that rule arrive with no `Content-Security-Policy` and no `X-Frame-Options`; pull request pages and the pull request list were not served by the worker and kept theirs. The rule ships widened, accepting that cost, since Chrome cannot tell a worker fetch for a frame from one for a tab. Whether a live issue page's site chrome answers the pull request page's selectors below is unverified.

A pull request page's site chrome is `.js-header-wrapper` around the `AppHeader` (or the logged-out `HeaderMktg`), `#repository-container-header`, and `footer.footer`; the pull request itself is in `main`. The conversation page is a React application whose tabs (Conversation, Commits, Checks, Files changed) are paths under `/pull/<n>`. Its title is the first `h1`. Its sticky header is a `PageHeader` whose class list carries `StickyPullRequestHeader-module__prHeader__<hash>`; it is `position: sticky; top: 0`, 60px tall, z-index 11, `display: none` until the page has scrolled past its header, when the page adds `StickyPullRequestHeader-module__is-stuck__<hash>` and it shows with `color-shadow-small`. Inside, the `StickyPullRequestHeader-module__prTitleArea__<hash>` row is a flex row 8px apart, aligned centre, with the state pill first and the `h2` title at `order: 1`; a child appended at `order: 2` with `margin-left: auto` lands at the row's right edge, which is where the overlay's panel puts its controls while the header is stuck. The hashes change per build, so the overlay matches the module name prefix and the `is-stuck` fragment, not the full class. The conversation is laid in a Primer `PageLayout` whose `prc-PageLayout-Content-<hash>` and `prc-PageLayout-ContentWrapper-<hash>` cap at 1012px and whose diff container is `.container-xl` at 1280px; with those caps lifted the timeline and sidebar filled a 1522px window.
