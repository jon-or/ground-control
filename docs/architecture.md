...
The server refuses any request carrying an `Origin` header, any `Host` other than its own loopback address, any target that is not origin-form, any `POST` that is not `application/json`, any body over 64 KB, and more than eight event streams. A web page can reach loopback, and the request that pushes configuration carries executable paths, so nothing a browser can send is accepted. The browser board is reached another way. Every refusal is written to `hub.log` with the header that decided it, while the client is told only that it was turned away: the first three happen before any route, so a client that is refused sees a status it cannot tell from a port some other program has taken — and if the hub does not write the reason down, a board that cannot reach the hub it can see leaves no evidence anywhere on the machine.
...
The snapshot is the developer's work in the open: session titles derived from their prompts, the absolute path of every checkout, branch names, issue and pull request titles, and the `gh` logins they declared. `hub.log` carries the same, and `config.json` carries the settings behind them — the repository, the logins, and the paths of the CLIs the hub spawns. Any process running as the developer can read `hub.json` and so the snapshot; the token keeps other users and web pages out, not the developer's own processes, and on Windows a file mode is not protection. Nothing leaves the machine. The Chrome extension keeps the last snapshot in `chrome.storage.session`, which Chrome clears when it exits, and nowhere else.
