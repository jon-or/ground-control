...
**Board state and interaction are local. Classification is not.** Everything the board reads, remembers and renders stays on this machine, and nothing about the developer's work is sent anywhere as a consequence of looking at a board. Card triage is the exception, and it is one the developer turns off in a setting: classifying a card sends that card's issue and pull request text — titles, bodies, and recent comments, other people's included — to Anthropic's API, because inference is cloud-based. It is the same trust the developer already extends by running Claude Code on the repository, made on their behalf by a background process rather than by them typing, which is why the setting exists and why the requirement states it (R38).
