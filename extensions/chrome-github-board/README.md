...
**Run only on boards filtered to my issues** (default on) keeps the overlay off any board whose filter does not name you: the filter must carry a positive assignee qualifier naming only `@me`, your signed-in login, or a login the hub reports. A login that is yours only because VS Code says so is recognized once the hub has answered, which the extension caches for later visits. It is off-limits to GitHub's own markup while off, exactly as a disallowed project is. A board filtered to you shows the same assignee on every card, which is what makes the pull request author worth showing in its place; on a wider board the assignee is still the useful face.
