import { CLAUDE_AGENT_ID } from './providers/claude.js';
import { basename, dirKey } from './paths.js';
import type { IdeWindow } from './ide.js';
...
export interface OpenRequest {
  ...
  /** R27: whether the board may bring another worktree's window forward, which moves the developer's focus. */
 ...
 */
function windowRoot(recorded: string | null, window: IdeWindow): string | null {
 ...
 */
export function planOpen(request: OpenRequest): OpenPlan {
  ...
  if (session.agent !== CLAUDE_AGENT_ID) {
    return {
      refusal: 'other-agent',
...
/**
 * The sessions the board offers to open. Every Claude session qualifies wherever it runs, because which window holds
 * it is read at the click rather than at the render.
 */
export function openableSessions(sessions: readonly Session[]): string[] {
  return sessions.filter((session) => session.agent === CLAUDE_AGENT_ID).map((session) => session.sessionId);
}
