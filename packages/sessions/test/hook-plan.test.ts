...
import type { HookPlan } from '../src/hookPlan.js';
...
const install = (settingsText: string | null): HookPlan =>
  planHookInstall({ settingsText, home: HOME, wanted: 'install' });

const remove = (settingsText: string | null): HookPlan =>
  planHookInstall({ settingsText, home: HOME, wanted: 'remove' });

const written = (plan: HookPlan): Record<string, unknown> => {
  if (plan.kind !== 'write') {
