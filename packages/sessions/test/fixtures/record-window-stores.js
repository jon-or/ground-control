...
// record of where a session can be reached. Read the diff before committing — a fixture is evidence.
const fs = require('node:fs');
const os = require('node:os');
const path = require('node:path');
const { DatabaseSync } = require('node:sqlite');
const { REPO, WORKTREES } = require('./anonymise.js');
...
function trimEditor(editor) {
  ...
  const keep = (entry) => {
    ...
    try { return JSON.parse(entry.value).providedId === 'claudeVSCodePanel'; } catch { return false; }
  };
...
function assertScrubbed(identifying, written) {
  ...
  // Separators vary with how deeply a path was escaped, so they are flattened, the synthetic ones struck out, and
  // whatever absolute path is left got through.
  let rest = json.split('\\').join('/').replace(/\/+/g, '/').toLowerCase();

  for (const prefix of SYNTHETIC) {
    rest = rest.split(prefix.toLowerCase()).join('');
  }

  // A drive letter, never the tail of a URI scheme: flattening leaves `file://` looking like one. The home directories
  // are how the same leak reads when the recording is made on macOS or Linux.
