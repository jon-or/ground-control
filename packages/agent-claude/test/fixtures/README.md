...
Markers are an internal format, so `phase.test.ts` constructs them directly. `hook-writer.test.ts` uses captured external payloads; its derived `notification_type`, nonempty `background_tasks`, and harness-prefixed `prompt` cases are identified in the test. Scrubbing replaces prompt text, so the recording cannot preserve a harness prefix. Scrubbing preserves event names, notification type, source, reason, tool name, background-task count, and presence of agent_id while replacing paths, IDs, prompts, and tool inputs.
