...
function main() {
  ...
  const files = cards.length > 0 ? NAMES.slice(0, cards.length) : NAMES.filter((n) => fs.existsSync(path.join(__dirname, n)));
  ...
  const recorded = files.map((name, i) =>
    cards.length > 0 ? record(repo, cards[i]) : JSON.parse(fs.readFileSync(path.join(__dirname, name), 'utf8')),
  );

  // One long body, so a fixture exercises clipping without anybody writing two kilobytes of prose by hand.
  const written = recorded.map((response, i) => anonymiseContext(structuredClone(response), logins, { longBody: i === 0 }));
