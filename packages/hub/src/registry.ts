...
import { DEFAULT_ACTIONS, DEFAULT_AVATAR_POLICY, DEFAULT_BOARD_POLICY, DEFAULT_CUSTODY, DEFAULT_IDLE_EXIT_MS, DEFAULT_LOGS, DEFAULT_NEW_SESSION, DEFAULT_SESSION_SCOPE, DEFAULT_TRIAGE, DEFAULT_WORKTREE, bootstrapDirOf } from '@ground-control/core';
...
export function makeRegistries(log?: Logger, injectedHome?: string, injectedEnv?: NodeJS.ProcessEnv, stateDir?: string): Registries {
  ...
  const codex = makeCodexAdapter({
    ...
    env: environment,
    // Dispatch output lands in the state directory of the injected home, not the developer's active one.
    start: makeMachineStarter(stateDir ?? bootstrapDirOf(home)),
    kill: killOnMachine,
