...
let cleanup: () => void;
const hubs: Hub[] = [];
beforeEach(() => { ({ home, dispose: cleanup } = tempHome()); home = home.replace(/\\/g, '/'); });
...
function profileAgent() {
  ...
  const adapter: AgentAdapter = {
    ...
    activity: {
      settingsPath: () => `${root}/settings.json`, watchDir: () => `${groundControlDirOf(home)}/fake-markers`, read: () => null,
      writer: { path: () => `${groundControlDirOf(home)}/writer.mjs`, source: 'cached writer' },
...
function harness(options: { env?: Record<string, string>; stored?: boolean; legacy?: boolean; failSave?: boolean } = {}) {
  ...
  const store = makeSettingsStore(home);
  ...
  const hub = new Hub({ home, registries, clock: clock.clock, log: captureLog().log, watch: () => ({ dispose() {} }),
    lanes: makeLaneStore(home), marks: makeMarkStore(home), triage: makeTriageStore(home), actions: makeActionStore(home),
    issues: makeIssueStore(home), status: makeStatusStore(home), checkouts: makeCheckoutStore(home),
...
describe('profile hook transitions', () => {
  it.each([false, true])('uses accepted custom homes for uninstall and refuses malformed config=%s', (malformed) => {
    ...
    const installed = syncActivity(regs.agents, 'install', home);
    ...
    makeSettingsStore(home).write({ ...defaultConfig(regs, fakeReaders({}, home)), agentHomes: { claude, codex } });
    if (malformed) writeFileSync(configPathOf(home), '{broken');
    const result = uninstallAgentActivity(home, { CLAUDE_CONFIG_DIR: `${home}/wrong-claude`, CODEX_HOME: `${home}/wrong-codex` });
    if (malformed) {
  ...
  function setup() {
    ...
    mkdirSync(`${groundControlDirOf(home)}/fake-markers`, { recursive: true });
    writeFileSync(`${groundControlDirOf(home)}/fake-markers/live.json`, 'cached marker');
    writeFileSync(`${groundControlDirOf(home)}/writer.mjs`, 'cached writer');
  ...
  it('removes only owned prior hooks before saving and preserves cached writers and markers', () => {
    ...
    expect(acceptAgentHomes(h.registries, h.before, h.after, home, () => {
    ...
    expect(readFileSync(`${groundControlDirOf(home)}/fake-markers/live.json`, 'utf8')).toBe('cached marker');
    expect(readFileSync(`${groundControlDirOf(home)}/writer.mjs`, 'utf8')).toBe('cached writer');
  });
  it.each(['malformed', 'unreadable', 'locked', 'save'])('refuses %s without accepting the new profile', (kind) => {
    ...
    if (kind === 'locked') writeFileSync(installLockPathOf(home), 'other installer');
    expect(acceptAgentHomes(h.registries, h.before, h.after, home, saved)).not.toBeNull();
  ...
  it('keeps both profile preimages when two backups share a timestamp', () => {
    ...
    const h = setup();
    expect(acceptAgentHomes(h.registries, h.before, h.after, home, () => {})).toBeNull();
    expect(syncActivity(h.registries.agents, 'install', home).failure).toBeNull();
    const backups = readdirSync(groundControlDirOf(home)).filter((name) => name.startsWith('settings-backup-fake-'));
    expect(backups).toHaveLength(2);
    expect(backups.sort().map((name) => JSON.parse(readFileSync(`${groundControlDirOf(home)}/${name}`, 'utf8'))))
  ...
  it('reports unreadable saved config and preserves it after a failed durable write', () => {
    const store = makeSettingsStore(home); const config = defaultConfig({ agents: [], hosts: [], sources: [] }, fakeReaders({}, home));
    store.write(config); const original = readFileSync(configPathOf(home), 'utf8');
    mkdirSync(`${configPathOf(home)}.${process.pid}.tmp`);
    expect(() => store.write({ ...config, installActivity: false })).toThrow();
    expect(readFileSync(configPathOf(home), 'utf8')).toBe(original);
  });
});
